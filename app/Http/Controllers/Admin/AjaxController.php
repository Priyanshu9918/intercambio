...
class AjaxController extends Controller
{
    ...
    */
    ...
    public function setStatusAction1(Request $request)
    {
        $table_name = $request->table_name;
        ...
        if ($request->ajax() && !empty($table_name) && !empty($id)) {
            ...
            if(stripos($type,'disable')!==false)
            {
                $query = DB::table($table_name)->where('id', $id)->update(['status' => 0,'updated_at' => date('Y-m-d H:i:s')]);
                DB::commit();
                $data['code'] = 200;
                $data['success'] = true;
                $data['type'] = $type;
            ...
            {
                $query = DB::table($table_name)->where('id', $id)->update(['status' => 1]);
                $data1 = DB::table($table_name)->where('id', $id)->first();
                $query = DB::table('teacher_pairings')->where('id', $data1->pairing_id)->update(['status' => 0,'updated_at' => date('Y-m-d H:i:s')]);
    ...
    public function setStatusAction2(Request $request)
    {
        $table_name = $request->table_name;
        ...
        if ($request->ajax() && !empty($table_name) && !empty($id)) {
            ...
            elseif(stripos($type,'enable')!==false)
            {
                $query = DB::table($table_name)->where('id', $id)->update(['status' => 1]);
                $data1 = DB::table($table_name)->where('id', $id)->first();
                $query = DB::table('teacher_pairings')->where('id', $data1->pairing_id)->update(['status' => 0]);
    ...
    public function subCategoryList(Request $request)
    {
        ...
        public function TimezoneLists(Request $request)
        {
            $id = $request->id;
            $state = DB::table('states')->where('id',$id)->first();

            $timezone = DB::table('time_zones')->where('id',$state->time_zone)->get();
